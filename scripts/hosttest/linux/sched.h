...
/* host-test shim: a task whose flags high bits carry our identity tag, plus the barriers. */
struct task_struct {
...
/* The host test includes policy.c into its own translation unit, so this is the one instance. */
...
/* host-test shim: the tag path is exercised through policy_lookup_as(), so this is unused. */
