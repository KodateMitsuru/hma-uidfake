 ...
 *   KAUX_CMD_SET:  attr KAUX_ATTR_BLOB = u32 npairs, then npairs * (caller,
 * target) caller == 0 means "any caller" KAUX_CMD_PING: no payload, ACK only
 *   KAUX_CMD_APK:  attr KAUX_ATTR_BLOB = u32 n, then n * (st_dev, ino_lo,
 * ino_hi, uid)
 */
#include "uidfake.h"
