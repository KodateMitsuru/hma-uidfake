...
/* HMA treats these as always-present system packages (defpackage/xj.java, field xj.a). */
...
std::set<std::string, std::less<>> HmaPolicy::string_set(const nlohmann::json& object,
                                                         std::string_view key) {
    std::set<std::string, std::less<>> result;
    const auto it = object.find(key);
    if (it == object.end() || !it->is_array()) return result;
    for (const auto& item : *it)
        if (item.is_string()) result.insert(item.get<std::string>());
    return result;
}
...
std::optional<HmaPolicy> HmaPolicy::load(const std::filesystem::path& path) {
    ...
    try {
        ...
        if (!in) return std::nullopt; /* sync_now() reports the outage once */
        in >> config;
...
Pairs HmaPolicy::expand(const PackageDb& packages) const {
    ...
    for (const auto& [name, caller] : scope.items()) {
        ...
        for (const auto& template_name : string_set(caller, "applyTemplates")) {
            ...
            if (it != templates.end())
                for (const auto& package : string_set(*it, "appList")) hidden.insert(package);
        }
        ...
        if (whitelist)
            ...
            for (const auto& builtin : kBuiltinPackages) hidden.erase(std::string{builtin});

        const bool exclude_system = caller.value("excludeSystemApps", false);
        const auto caller_uid = packages.uid_of(name);
        if (!caller_uid || *caller_uid < kFirstAppUid) continue;
        ...
        for (const auto& [target_name, target_uid] : packages.by_name()) {
            ...
            if (whitelist ? listed : !listed) continue;
            if (exclude_system && packages.is_system(target_name)) continue;
            if (target_uid < kFirstAppUid) continue;
