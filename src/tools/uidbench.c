 ...
 *   H2  getpriority(PRIO_USER, hidden2)      hooked, policy hit   (null pair vs H1)
 *   AB  getpriority(PRIO_USER, absent)       hooked, policy miss
 *   C1  getpriority(PRIO_PROCESS, dead pid)  NOT hooked: pid hash miss -> ESRCH
 *   C2  ioprio_get(IOPRIO_WHO_PROCESS, dead) NOT hooked
 *
 * C1 is the "similar sized call": same syscall, same shape (entry + one hash lookup
 * that misses + ESRCH), but it never reaches find_user(), so our hook is not on it.
 * C2 is a second, different-syscall baseline.
 *
 ...
 *   - Welch t for H1 vs AB (the primary test), H1 vs H2 (the noise floor), H1 vs C1;
 *   - the number of paired samples an attacker needs for one 5 sigma decision,
 *     (5*sd/delta)^2, which is what decides whether a difference is exploitable;
 *   - the ratio mean(H1)/mean(C1) as a sanity check that the two calls really are of
 *     similar size. The two paths are not isomorphic, so this one is reported with a
 *     deliberately loose band and only warns.
 *
 * Run it as one of the app uids that hide (the caller of a lookup must be an app uid for
 * the policy to match at all), for example from a probe app, or with run-as.
 *
 *   uidbench --hidden 10400,10401 --absent 10500 --rounds 200000 --block 64 --pin 4
 *
 * It can also be built as a shared library (-DUIDBENCH_LIB) and called from inside an app:
 * an app cannot exec a file from its data directory, and running it as the shell uid would
 * not match the policy, because only app uids (10000 + appid + user*100000) can be callers.
 *
...
static const char *cl_name[] = { "H1 hidden1", "H2 hidden2", "AB absent ", "AB2 absent2", "C1 getprio(pid)", "C2 ioprio(pid)" };
...
static double welch_t(double m1, double s1, size_t n1, double m2, double s2, size_t n2)
{
	double d = sqrt(s1 * s1 / (double)n1 + s2 * s2 / (double)n2);

	return d > 0 ? (m1 - m2) / d : 0.0;
}
...
struct stat_t { double mean, sd; size_t n; };
...
/* does the hidden uid take the fast (miss) path? */
static int probe_hidden_ok(unsigned long uid, double base_ns)
{
	return probe_ns(PRIO_USER_NR, uid, 200) <= 4.0 * base_ns;
}

/*
 * The absent controls must be uids that do not exist: an existing uid takes the "user exists"
 * path and walks every process, which is ~1000x slower and makes the whole run meaningless
 * (and slow). Scan the app range and keep the fast ones, which are the uids Android has not
 * handed out.
 */
static int pick_absent(unsigned long *out, int want, double limit)
{
	unsigned long u;
	int n = 0;

	for (u = 10000; u <= 19999 && n < want; u += 7) {
		if (probe_ns(PRIO_USER_NR, u, 4) < limit)
			out[n++] = u;
	}
	return n;
}
...
int uidbench_main(int argc, char **argv)
{
	...
	/* sanity probe: a hidden uid that exists and is not hooked takes the 'uid exists' path,
	 * which walks every process (measured 674 us against 0.45 us) and ruins the run */
	{
		double base_ns = probe_ns(PRIO_PROCESS_NR, dead, 200);

		if (probe_ns(PRIO_USER_NR, hidden1, 200) > 4.0 * base_ns && !force) {
			fprintf(stderr, "uidbench: hidden uid %lu takes the slow 'uid exists' path.\n"
				"  Add this process's uid (%d) to the policy's caller list, or load the "
				"policy. (--force overrides)\n", hidden1, (int)getuid());
			return 3;
		}
		for (i = 0; i < n_absent; i++) {
			if (probe_ns(PRIO_USER_NR, absent_list[i], 8) > 4.0 * base_ns) {
				fprintf(stderr, "uidbench: absent uid %lu exists in the kernel; "
	...
	fprintf(stderr, "uidbench: %ld rounds x %ld calls x %d classes = %.3g syscalls\n",
	...
	for (r = 0; r < rounds; r++) {
		cl_arg[CL_AB] = absent_list[(int)((unsigned long)r % (unsigned long)n_absent)];
		cl_arg[CL_AB2] = absent_list[(int)(((unsigned long)r + 1) % (unsigned long)n_absent)];
		t0 = (double)now_ns();
		for (i = 0; i < CL_N; i++) {
		...
		if (progress && r >= next) {
			...
			fprintf(stderr, "\r  %5.1f%%  %ld/%ld rounds  %.0fs elapsed  eta %.0fs      ",
				frac * 100.0, r + 1, rounds, el, frac > 0 ? el / frac * (1.0 - frac) : 0.0);
...

	{
		...
		printf("uidbench: hidden %lu,%lu  absent %d uid(s)  dead pid %lu  rounds %ld  block %ld"
		       "%s%d\n", hidden1, hidden2, n_absent, dead, rounds, block,
		       pin >= 0 ? "  pin " : "", pin);
		printf("  warmup %.0f%% dropped, %.1f%% trimmed per tail, %zu samples used\n",
		...
		printf("    H1 - AB  : %+7.3f ns  sd %6.2f  t %8.2f   <- signal (hidden vs absent)\n",
		       dab.mean, dab.sd, tab2 * 0 + (dab.sd > 0 ? dab.mean / (dab.sd / sqrt((double)dab.n)) : 0.0));
		printf("    AB - AB2 : %+7.3f ns  sd %6.2f  t %8.2f   <- reference: plain uid spread\n",
		       dab2.mean, dab2.sd, tab2);
		printf("    H1 - H2  : %+7.3f ns  sd %6.2f  t %8.2f   <- hidden vs hidden\n",
		       dh2.mean, dh2.sd, th2);
		printf("    H1 - C1  : %+7.3f ns  sd %6.2f  t %8.2f   <- hooked vs unhooked path\n",
		       dc1.mean, dc1.sd, dc1.sd > 0 ? dc1.mean / (dc1.sd / sqrt((double)dc1.n)) : 0.0);

		ratio = c1.mean > 0 ? h1.mean / c1.mean : 0.0;
		printf("\n  similar-size sanity   mean(H1)/mean(C1) = %.3f   (band %.2f, loose by design)\n",
		       ratio, ratio_band);
		printf("  signal / reference ratio : %.3f  (<< 1 means the hook hides inside the natural\n"
		       "                                       uid-to-uid spread of find_user())\n",
		       fabs(dab2.mean) > 0 ? fabs(dab.mean) / fabs(dab2.mean) : 0.0);

		printf("\n  verdict\n");
		printf("    signal H1-AB    : %+.3f ns   95%% CI %+.3f .. %+.3f  (sem %.3f ns)\n",
		       dab.mean, dab.mean - 1.96 * dab.sd / sqrt((double)dab.n),
		       dab.mean + 1.96 * dab.sd / sqrt((double)dab.n), dab.sd / sqrt((double)dab.n));
		printf("    reference AB-AB2: %+.3f ns  sd %.3f ns  (3 sigma = %.3f ns)\n",
		       dab2.mean, dab2.sd, 3.0 * dab2.sd);
		printf("    hidden vs absent   : %s  (|signal| %.3f vs tolerances 3sd=%.3f, %.3f ns)\n",
		       (fabs(dab.mean) <= 3.0 * dab2.sd || fabs(dab.mean) <= max_ns) ? "PASS" : "FAIL",
		       dab.mean, 3.0 * dab2.sd, max_ns);
		printf("    exploitable        : %s\n",
		       (fabs(dab.mean) <= 3.0 * dab2.sd || fabs(dab.mean) <= max_ns) ? "no" : "YES");
			...
			if (f) {
				...
				for (r = 0; r < rounds; r++)
					fprintf(f, "%ld,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n", r,
						v[CL_H1][r], v[CL_H2][r], v[CL_AB][r], v[CL_C1][r],
						d_ab[r], d_h2[r], d_c1[r], d_ab2[r]);
				fclose(f);
		...
		return (fabs(dab.mean) <= 3.0 * dab2.sd || fabs(dab.mean) <= max_ns) ? 0 : 1;
	}
}
...
{
	return uidbench_main(argc, argv);
}
#endif
