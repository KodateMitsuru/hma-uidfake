...
static struct hook_entry g_hook[] = {
	...
	{ __NR_setuid, uidfake_setuid, NULL },
	{ __NR_setreuid, uidfake_setreuid, NULL },
	{ __NR_setresuid, uidfake_setresuid, NULL },
...
/*
 * Called from do_dentry_open() for every file a task opens -- openat, openat2,
 * creat, all of them -- with the struct file already filled in. It is the
 * vendor hook AOSP keeps for exactly this (include/trace/hooks/syscall_check.h:
 * "a mechanism for vendor modules to hook and extend functionality"), so the
 * module no longer needs an entry in sys_call_table to see opens.
 *
 * The first thing checked is one bit of the task's flags, so a process that is
 * not waiting for a name pays an AND here and nothing else.
 */
/*
					   ...
					   int depth)
{
	...
	pr_info("uidfake: no rule: %s in %s/%s (ino %lu dev %u) depth %d\n",
		current->comm, file->f_path.dentry->d_parent->d_name.name,
...
static void uidfake_file_open(void *data, const struct file *file)
{
	...
	/*
   * Only an open that lands on the filesystem an app's code lives on says
   * anything at all: while the framework sets the process up it reads
   * properties, /proc, /dev and /system, and those are none of our business.
   * Without this gate the very first property read would end the wait.
   */
	if (!uidfake_dev_is_code(inode->i_sb->s_dev))
		return;

	for (dentry = file->f_path.dentry; dentry && depth < UF_DIR_DEPTH;
	     depth++) {
		const struct inode *ancestor = d_inode(dentry);

		if (!ancestor)
			continue;
		tag = uidfake_apk_lookup(ancestor->i_sb->s_dev,
					 (u64)ancestor->i_ino);
		if (tag)
			break;
	...
	if (tag) {
		...
		if (UF_DEBUG_ON())
			uidfake_close_unheard(file, (u32)inode->i_sb->s_dev,
					      depth);
		uidfake_tag_close();
	}
}
...
/*
 * The id setters. Their work happens on the way back, on the syscall's return
 * value, which is why they need their own helper. All twelve wrappers (uid and
 * gid, native and compat) run the same code, so no member of the family stands
 * out, and a gid syscall cannot tag anything because uidfake_tag_adopt() reads
 * the fsuid, which only the uid syscalls change.
 */
/* security_cred_getsecid() is exported; its declaration is not in every header
 * set we use. */
extern void security_cred_getsecid(const struct cred *cred, u32 *secid);

/*
 * The id setters. The SIDs are captured around the call so that the two ends of
 * an identity change can be linked: the SID map that identifies isolated
 * processes is built from here.
 */

/*
 * The id setters. Both ends of a transition are captured so the SID map that
 * identifies isolated processes can be built from it; the before side has to be
 * read before the syscall runs, because the transition itself changes it. Work
 * is skipped when nothing can be learned: only a change that ends on an app uid
 * or an isolated uid matters.
 */
